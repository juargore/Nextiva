...
import com.nextiva.nextivaapp.android.databinding.FabContainerBinding
import com.nextiva.nextivaapp.android.databinding.FloatingActionButtonBinding
import com.nextiva.nextivaapp.android.fab.extensions.clearParentAlignmentRules


@Suppress(
	"MemberVisibilityCanBePrivate",
	"unused"
) // because we want to expose the methods to end users
class FloatingActionButton : RelativeLayout {
	...
	private lateinit var contentCover: View
	private lateinit var fabCard: View
	private lateinit var fabIconWrapper: LinearLayout
	...
	private fun bindViews() {
		...
		val fabContainerBinding = FabContainerBinding.inflate(inflater, this, true)
		val floatingActionButtonBinding = FloatingActionButtonBinding.bind(fabContainerBinding.root)

		container = fabContainerBinding.container
		contentCover = fabContainerBinding.contentCover
		fabCard = floatingActionButtonBinding.fabCard
		fabIconWrapper = floatingActionButtonBinding.fabIconWrapper
	}
	...
	private fun applyAttributes(rawAttrs: AttributeSet?) {
		val attrs =
			context.theme.obtainStyledAttributes(rawAttrs, R.styleable.FloatingActionButton, 0, 0)
		try {
			setButtonPosition(
				attrs.getInteger(
					R.styleable.FloatingActionButton_buttonPosition,
					buttonPosition
				)
			)
			setButtonBackgroundColour(
				attrs.getColor(
					R.styleable.FloatingActionButton_buttonBackgroundColour,
					buttonBackgroundColour
				)
			)
			setButtonIconResource(
				attrs.getResourceId(
					R.styleable.FloatingActionButton_buttonIcon,
					0
				)
			)
			setInternalOffsetTop(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetTop,
					0f
				)
			)
			setInternalOffsetBottom(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetBottom,
					0f
				)
			)
			setInternalOffsetStart(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetStart,
					0f
				)
			)
			setInternalOffsetEnd(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetEnd,
					0f
				)
			)
			setInternalOffsetLeft(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetLeft,
					0f
				)
			)
			setInternalOffsetRight(
				attrs.getDimension(
					R.styleable.FloatingActionButton_internalOffsetRight,
					0f
				)
			)
		} finally {
	...
	private fun setSpeedDialMenuItemViewOrder(view: ViewGroup) {
		...
		val label = view.findViewById<TextView>(R.id.menu_item_label)
		val icon = view.findViewById<ViewGroup>(R.id.menu_item_card)
	...
	fun setButtonPosition(position: Int) {
		...
		setViewLayoutParams(fabCard)
		setViewLayoutParams(contentCover)
	...
	fun setButtonBackgroundColour(@ColorInt colour: Int) {
		...
		if (Build.VERSION.SDK_INT >= 21) {
			(fabCard as CardView).setCardBackgroundColor(colour)
		} else {
			(fabCard.background as GradientDrawable).setColor(colour)
		}
	}

	fun setButtonIconResource(@DrawableRes icon: Int) {
		buttonIconResource = icon
		fabIconWrapper.setBackgroundResource(icon)
	}
	...
	fun setContentCoverColour(@ColorInt colour: Int) {
		...
		(contentCover.background as GradientDrawable).setColor(colour)
	}
	...
	fun rebuildSpeedDialMenu() {
		...
		for (i in (0 until adapter.getCount())) {
			...
			val menuItemLabel = view.findViewById<TextView>(R.id.menu_item_label)
			val menuItemCard = view.findViewById<ViewGroup>(R.id.menu_item_card)
			val menuIconWrapper = view.findViewById<LinearLayout>(R.id.menu_item_icon_wrapper)

			menuItemLabel.text = menuItem.getLabel()
			speedDialMenuAdapter?.onPrepareItemLabel(context, i, menuItemLabel)

			(menuItemCard.background as Drawable).setTint(adapter.getBackgroundColour(i))
			speedDialMenuAdapter?.onPrepareItemCard(context, i, menuItemCard)

			menuIconWrapper.background = menuItem.getIcon()
			speedDialMenuAdapter?.onPrepareItemIconWrapper(context, i, menuIconWrapper)
	...
	private fun toggleSpeedDialMenu() {
		...
		contentCover.isClickable = isSpeedDialMenuOpen
		contentCover.isFocusable = isSpeedDialMenuOpen
		if (isSpeedDialMenuOpen) {
			contentCover.setOnClickListener { toggleSpeedDialMenu() }
		} else {
	...
	private fun animateFabIconRotation() {
		...
		fabIconWrapper.animate()
			.rotation(
				if (isSpeedDialMenuOpen) speedDialMenuAdapter?.fabRotationDegrees() ?: 0F else 0F
			)
	...
	private fun animateContentCover() {
		...
		if (isSpeedDialMenuOpen) {
			...
			coverScale = 2 * windowDiagonal.toFloat() / fabCard.width
		}
	...
	private fun animateSpeedDialMenuItems(immediate: Boolean = false) {
		...
		val distance = fabCard.height.toFloat()
